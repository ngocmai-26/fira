...
import { deleteRoles, getAllRole, getRoleById, updatePermission } from "../../../thunks/RolesThunk";
      ...
      <Layout>
        ...
        <div className="p-4">
          ...
          <div className="flex justify-between">
            <SearchComponent placeholder="Nhập tên chức vụ" style={"w-2/6"} />
              ...
              {allRole?.map((item, key) => (
                <tr className="hover:bg-gray-100" key={key}>
                  ...
                  <td className="p-4 text-base font-medium text-gray-900 whitespace-nowrap">
                    <button className="w-full" onClick={() => handleGetRoleById(item?.id)}>{item?.roleName}</button>
                  </td>
                  <td className="p-4 text-base font-medium text-gray-900 whitespace-nowrap">
                    {item?.description}
                  </td>
        ...
        <div
        className={`fixed mx-auto ${showRoleById? "block": "hidden"} left-0 right-0 z-50 items-center justify-center  overflow-x-hidden overflow-y-auto top-4 md:inset-0 h-modal sm:h-full`}
        id="edit-user-modal"
      >
                        ...
                        <div className="border-2 h-40 overflow-y-auto overflow-hidden">
                          <ul>
                            {allPermission?.map((pre, key) =>
                              taskList?.map((item) =>
                                pre.id === item ? (
                                  <li className="text-xs p-1 flex justify-between py-1.5" key={key}>
                                    {pre.name}
                                    <button type="button" onClick={() => handleRemoveTaskList(pre.id)}>X</button>
                                  </li>
                                ) : (
                                  <li></li>
                                )
